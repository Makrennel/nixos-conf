{
  ...
  swap-size ? builtins.readFile (pkgs.runCommand "get-swap-size" {} "grep MemTotal /proc/meminfo | sed 's/[^0-9]*//g' | read mem; echo $[ $mem / 1024.0 ]'M' > $out"), # Same size as RAM by default
